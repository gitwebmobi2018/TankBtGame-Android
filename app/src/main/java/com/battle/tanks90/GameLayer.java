...
public class GameLayer implements IConstant {
	...
	public static boolean HAVE_PHYSIC_LAYER = true;
	...
	public static void drawLayerBackGround() {
		...
		for (int indexlayer = 0; indexlayer < arrayListDataMap.size() - offsetDrawPhysicMap; indexlayer++) {
			for (int row = 0; row <= GameLib.SCREEN_HEIGHT / TILE_HEIGHT  + 1; row++)
				for (int col = 0; col <= GameLib.SCREEN_WIDTH / TILE_WIDTH + 1; col++) {
					if (col + begincol < MAX_TILE_COLUMN && row + beginrow < MAX_TILE_ROW) {

						if (arrayListDataMap.get(indexlayer)[row + beginrow][col + begincol] > -1)
							GameLib.mainCanvas.drawBitmap(arrayListBitmap.get(arrayListDataMap.get(indexlayer)[row + beginrow][col + begincol]), offsetx + col * TILE_WIDTH, offsety + row * TILE_HEIGHT, null);
					}
				}
		}
	}
